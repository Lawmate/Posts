/**
  Copyright (C) 2012-2026 by Lorro Precision Ltd.
  All rights reserved.

  CMZ Lathe post processor configuration.
...
description = "CMZ TA-25-YS-640 v2";
vendor = "CMZ";
vendorUrl = "https://www.cmz.com";
legal = "Copyright (C) 2012-2026 by Lorro Precision Ltd";
certificationLevel = 2;
minimumRevision = 45909;

longDescription = "CMZ lathe (Fanuc 32i control) post with support for mill-turn for use with TA.";
...
properties = {
  machineModel: {
    ...
    type       : "enum",
    values     : [
      {title:"TA with Y-axis", id:"TA_YAXIS"},
    ],
    value: "TA_YAXIS",
    scope: "post"
  },
  ...
  barLoader: {
    title      : "Insert top cut and remnant eject blocks for use with bar loader",
  ...
  safeParting: {
    title      : "Safe parting position",
    description: "If bar pulling and parting, position the parting blade at the back of the part after op1, before pulling the bar.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  oscCut: {
    ...
    description: "Turn on or off oscillation cutting, used for breaking chips. Selecting it here will turn it on for every strategy including finishing cuts. To select it for individual strategies, write a strategy note 'osc' or if selected here, write a note 'noosc' to turn it off for that one",
    group      : "preferences",
    type       : "enum",
    values     : [
      {title:"On", id:"true"},
  ...
  oscIVal: {
    title      : "Oscillation cutting I val frequency",
    description: "The frequency with which the oscillation occurs.",
    group      : "preferences",
    type       : "number",
    value      : 1.0,
    scope: "post"
  ...
  oscKVal: {
    title      : "Oscillation cutting K val amplitude",
    description: "The amplitude with which the oscillation occurs.",
    group      : "preferences",
    type       : "number",
    value      : 1.2,
    scope: "post"
  ...
  homePositionB: {
    title      : "B-home position",
    description: "B-home position for the G53/WCS X-axis home positioning methods to enable safe turret rotation.",
    group      : "homePositions",
    type       : "number",
    value      : 0, 
    scope      : "post"
  ...
  homePositionX: {
    title      : "X-home position",
    description: "X-home position for the G53/WCS X-axis home positioning methods to enable safe turret rotation.",
    group      : "homePositions",
    type       : "number",
    value      : -50, 
    scope      : "post"
  ...
  homePositionY: {
    ...
    description: "Y-home position for the G53/WCS Y-axis home positioning methods.",
    group      : "homePositions",
    type       : "number",
    value      : 0, 
    scope      : "post"
  ...
  useExtendedOffsets: {
    ...
    description: "Select whether extended offsets are supported on the main tool holder, the lower turret, or both. 'Machine default' will enable extended offsets on the TA main spindle.",
    group      : "preferences",
    type       : "enum",
    values     : [
...
//   {id:COOLANT_HIPRESSURE, on:18, off:19},
  ...
  {id:COOLANT_FLOOD_THROUGH_TOOL, on:"M18 (High pressure on)", off:"M17 (High pressure off)"},
...
var threadPQFormat = createFormat({decimals:(unit == MM ? 4 : 5), type:FORMAT_LZS, minDigitsLeft:0, minDigitsRight:1});
var dwellFormat = createFormat({prefix:"U", decimals:3});
// var peckFormat = createFormat({decimals:(unit == MM ? 3 : 4), type:FORMAT_REAL});
var peckFormat = createFormat({decimals:(unit == MM ? 3 : 4), type:FORMAT_LZS, minDigitsLeft:0, minDigitsRight:(unit == MM ? 4 : 4)});
...
var hOutput = createOutputVariable({prefix:"H"}, zFormat);
var rOutput = createOutputVariable({prefix:"R"}, zFormat);
...
var lOutput = createOutputVariable({prefix:"L", control:CONTROL_FORCE}, rpmFormat);
...
*/
  case "ENABLE_C_AXIS":
    ...
    return (spindle == SPINDLE_MAIN) ? 80 : 280;
  case "DISABLE_C_AXIS":
    machineState.cAxisIsEngaged = false;
    return (spindle == SPINDLE_MAIN) ? 81 : 281;
  ...
  case "CYCLE_PART_EJECTOR":
    return 247;
  case "AIR_BLAST_ON":
    return (spindle == SPINDLE_MAIN) ? 54 : 254;
  case "AIR_BLAST_OFF":
...
function defineMachine() {
  ...
  if (getProperty("machineModel") == "TA_YAXIS") {
    modelType = "TA-25-YS-640";
    turret1GotYAxis = true;
    yAxisMinimum = toPreciseUnit(-70, MM); // specifies the minimum range for the Y-axis
  ...
  // } else if () {
    ...
    error(localize("Machine type not configured"));
  }
...
function activateMachine(section) {
  ...
  machineConfiguration.setVendor("CMZ");
...
//function to format the text of the filename into the top of the output
function toCamelCase(str) {
  return str
    ...
    .replace(/^(.)/, function(match, chr) {
      return chr.toUpperCase(); // uppercase instead of lower
    });
}
...
function onOpen() {
  ...
  //Output nc file name as file label for fanuc
  var rawName = FileSystem.getFilename(getOutputPath()).split(".")[0];
  var camelName = rawName ? toCamelCase(rawName) : "program";
  writeln("<" + camelName + ">");

  // var notez = getParameter("job-notes");
  writeComment(jobNotes);
  // writeComment("here");
  // writeComment((getParameter("notes")?getParameter("notes"):"no notes"));
  if(getProperty("job-notes")=="osc"){
    writeComment("osc required");
  }
...
// writeComment("Define offsets");
//G53
// writeBlock(gFormat.format(10), pOutput.format(0), xOutput.format(0), yOutput.format(0), zOutput.format(0), formatComment("g53"));
// //G54 - main spindle
// writeBlock(gFormat.format(10), lOutput.format(2), pOutput.format(1), zOutput.format(133), cOutput.format(0), formatComment("g54"));
// //G53 - sub spindle
// writeBlock(gFormat.format(10), lOutput.format(2), pOutput.format(2), zOutput.format(579.6943), cOutput.format(0), formatComment("g55"));
writeBlock(gFormat.format(0), formatComment("rapid move mode"));
writeBlock(gFormat.format(18), formatComment("YZ plane mode"));
writeBlock(gFormat.format((unit == IN) ? 20 : 21), formatComment("set units"));
writeBlock(gFormat.format(40), formatComment("disable tool radius compensation"));
writeBlock(gFormat.format(54), formatComment("offset g54"));
writeBlock(gFormat.format(80), formatComment("disable drilling cycles"));


  if (getProperty("gotChipConveyor")) {
    writeComment("chip conveyor on");
...
/2T1010(PART OFF) 
/2M81 
/2M10(CLOSE MAINSPINDLE)
/2G50S1000
/2G96S120M3 
/2G0Z0.3(FACING ALLOWANCE FIRST END)
/2G0X35.Y0M8(BAR DIA+3MM)(COOLANT ON) 
/2(M66)(TOOL MONITORING ON) 
/2G1X3.F.10 
/2X-1.0F.06 
/2(M67)(TOOL MONITORING OFF)
/2W1.0
/2G0X100.M9 
/2G53Y0 
/2G53X-50 
...
//top cut code
if (getProperty("barLoader")==true){

  writeBlock("/2"+gFormat.format(0), formatComment("rapid move"));
  writeBlock("/2"+gFormat.format(18), formatComment("g18 plane"));
  writeBlock("/2"+gFormat.format(40), formatComment("disable compensation"));
  writeBlock("/2"+gFormat.format(80), formatComment("disable drilling cycles"));
  writeBlock("/2"+gFormat.format(99), formatComment("feed per revolution"));
  subOutput.reset();
  yOutput.reset();
  xOutput.reset();
  zOutput.reset();
  writeBlock("/2"+gFormat.format(53), gMotionModal.format(0), subOutput.format(getProperty("homePositionB")), formatComment("turret safe position b"));
  writeBlock("/2"+gFormat.format(53), gMotionModal.format(0), yOutput.format(getProperty("homePositionY")), formatComment("turret safe position y"));
  writeBlock("/2"+gFormat.format(53), gMotionModal.format(0), xOutput.format(0.5*getProperty("homePositionX")), formatComment("turret safe position x"));
  writeBlock("/2"+gFormat.format(53), gMotionModal.format(0), zOutput.format(getProperty("homePositionZ")), formatComment("turret safe position z"));
  writeBlock("/2"+gFormat.format(54), formatComment("spindle 1 offset"));
  writeBlock("/2"+"T"+toolFormat.format(1010), formatComment("part off tool"));
  writeBlock("/2"+mFormat.format(81), formatComment("c axis release"));
  writeBlock("/2"+mFormat.format(10), formatComment("close main chuck"));
  writeBlock("/2"+gFormat.format(50), sOutput.format(1000), formatComment("set top speed to 1000rpm"));
  writeBlock("/2"+gFormat.format(96), sOutput.format(120), mFormat.format(3), formatComment("feed per rev and start spindle"));
  gMotionModal.reset();
  writeBlock("/2"+gMotionModal.format(0), zOutput.format(0.3), formatComment("parting blade position"));
  writeBlock("/2"+mFormat.format(73), formatComment("parts catcher up to chuck"));
  gMotionModal.reset();
  writeBlock("/2"+gMotionModal.format(0), xOutput.format(0.5*35), mFormat.format(8), formatComment('parting blade position for 1-1/4 inch bar'));
  writeBlock("/2"+gMotionModal.format(1), xOutput.format(0.5*3), feedOutput.format(0.1), formatComment('parting blade cut to 3mm'));
  writeBlock("/2"+gMotionModal.format(1), xOutput.format(0.5*-1), feedOutput.format(0.06), formatComment('parting blade part off'));
  writeBlock("/2"+gMotionModal.format(0), xOutput.format(0.5*100), mFormat.format(9), formatComment('rapid out'));
  writeBlock("/2"+mFormat.format(74), formatComment("parts catcher up to conveyor"));
  writeBlock("/2"+gFormat.format(53), gMotionModal.format(0), yOutput.format(getProperty("homePositionY")), formatComment("turret safe position y"));
  writeBlock("/2"+gFormat.format(53), gMotionModal.format(0), xOutput.format(0.5*getProperty("homePositionX")), formatComment("turret safe position x"));
  writeBlock("/2"+gFormat.format(53), gMotionModal.format(0), zOutput.format(getProperty("homePositionZ")), formatComment("turret safe position z"));
}
...
function invertAxes(activate, polarMode) {
  ...
  hOutput.setFormat(zFormat);
  rOutput.setFormat(zFormat);
...
function onSection() {
  ...
  // if(isFirstSection()){
    ...
    // writeComment("jobnote: " + hasParameter("job-notes"));
  ...
  if(getProperty("oscCut")==true){
    oscCutting = true;
  }

  if(oscCutting){
    oscIVal = Number(getProperty("oscIVal"));
    oscKVal = Number(getProperty("oscKVal"));
    // writeComment("ival " + oscIVal);
    // writeComment("kval " + oscKVal);
  }
  ...
  if (!isFirstSection() && insertToolCall) {
    ...
    if (currentId > 0) {
        var previousStrategy = getSection(currentId - 1).getStrategy();
        // writeComment(previousStrategy);
        if(previousStrategy=="turningSecondarySpindlePull"){
          isBarPull = true;
        }
    }
  ...
  if (hasParameter("operation-comment")) {
    ...
    if (comment) {
        ...
        var toolinf = getSection(getCurrentSectionId()).getTool();
        var toolOffsetFactorz = getToolOffsetFactor(toolinf);
        var compensationOffsetz = toolinf.isTurningTool() ? toolinf.compensationOffset : toolinf.lengthOffset;
        var commentz = "T" + toolFormat.format(toolinf.number * toolOffsetFactorz + compensationOffsetz % toolOffsetFactorz) + 
        " - " + toolinf.description;
        writeComment(commentz);
    }
  }
    ...
    if (insertToolCall || wcsOut || feedMode) {
      ...
      if (!machineState.spindlesAreAttached) {
        if (getProperty("machineModel") == "PUMA_SMX") {
          ...
          writeBlock(feedMode, formatComment("Set feed mode")); 
          writeBlock(gPlaneModal.format(plane), formatComment("Set work plane")); 
          writeBlock(cAxisEngageModal.format(getCode("ENABLE_C_AXIS", getSpindle(PART))), formatComment("Engage C axis"));
        }
        // onCommand(COMMAND_UNLOCK_MULTI_AXIS);
        // unwindCAxis();
        if (!machineState.usePolarInterpolation && !machineState.usePolarCoordinates && !currentSection.isMultiAxis()) {
  ...
  // Turning is active
    ...
    //   onCommand(COMMAND_UNLOCK_MULTI_AXIS);
      writeBlock(wcsOut, formatComment("Offset g54 sp1, g55 sp2"));
      writeBlock(feedMode, formatComment("feed per rev"));
      writeBlock(gPlaneModal.format(18), formatComment("XZ plane"));
      writeBlock(cAxisEngageModal.format(getCode("DISABLE_C_AXIS", getSpindle(PART))), formatComment("Disable both C axes"));
    } else {
      if (wcsOut) {
        writeBlock(wcsOut);
      }
  ...
  if ((maximumSpindleSpeed > 0) && (currentSection.getTool().getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED)) {
    if ((insertToolCall || rpmFormat.areDifferent(maximumSpindleSpeed, previousMaximumSpeed)) && !machineState.stockTransferIsActive) {
      writeBlock(gFormat.format(50), sOutput.format(maximumSpindleSpeed), formatComment("set spindle max speed"));
      sOutput.reset();
  ...
  if (insertToolCall) {
    ...
    // var tool = getSection(getCurrentSectionId()).getTool();
    // var toolOffsetFactor = getToolOffsetFactor(tool);
    // var compensationOffset = tool.isTurningTool() ? tool.compensationOffset : tool.lengthOffset;
  ...
  // writeComment(machineState.stockTransferIsActive);
  ...
  if (spindleChanged || isParting) {
    ...
    // Turn spindle on
    // writeComment(getSection(getCurrentSectionId()).getTool().spindleRPM);
    //check to see if a spindle speed has been set. If 0, it is constant surface speed
    var toolRPM = getSection(getCurrentSectionId()).getTool().spindleRPM;
    var toolCSS = (tool.getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED);
    // writeComment(toolCSS);
    if (!tapping && toolCSS) {
    ...
    if (isParting){
      writeBlock(
        gFormat.format(50),
        sOutput.format(tool.maximumSpindleSpeed),
        formatComment("set parting max spindle speed")
      );
      // writeBlock(
      //   gFormat.format(96),
      //   sOutput.format(Number(tool.surfaceSpeed)*0.001),
      //   mFormat.format(3),
      //   formatComment("set parting surface speed")
      // );
    }
  }
  ...
  if (getSpindle(PART) == SPINDLE_SUB) {
    // writeBlock(mInterferModal.format(getCode("INTERFERENCE_CHECK_OFF", getSpindle(PART))), formatComment("interference check off"));
  }
...
function goHome() {
  ...
  writeBlock(gMotionModal.format(0), gFormat.format(53), xOutput.format(getProperty("homePositionX")/2), yOutput.format(getProperty("homePositionY")), formatComment("turret safe position"));
...
function getLinearDirection(x, y, z) {
  var current = getCurrentPosition();
  
  var dx = Math.abs(x - current.x);
  var dz = Math.abs(z - current.z);
...
function oscOff(){
  writeBlock(gMotionModal.format(8.5), "P0", formatComment("OC off"));
  oscOn = false;
}
...
function oscOnZ(){
  // forceXYZ();
  gMotionModal.reset();
  // writeComment((currentSection.spindle == SPINDLE_SECONDARY)?"subspindle":"mainspindle");
  writeBlock(gMotionModal.format(8.5), "P2", "Z0", iOutput.format(oscIVal), kOutput.format((currentSection.spindle == SPINDLE_SECONDARY)?-oscKVal:oscKVal), formatComment("OC on in Z"));
  oscOn = true;
  prevOsc = "z";
}
...
function oscOnX(){
  // forceXYZ();
  gMotionModal.reset();
  writeBlock(gMotionModal.format(8.5), "P2", "X0", iOutput.format(oscIVal), kOutput.format((currentSection.spindle == SPINDLE_SECONDARY)?-oscKVal:oscKVal), formatComment("OC on in X"));
  oscOn = true;
  prevOsc = "x";
}
...
function onLinear(_x, _y, _z, feed) {
  ...
  //if using oscillation cutting, switch it on and off in the direction of the move.
  if(oscCutting){
    var direction = getLinearDirection(_x, _y, _z);
    if (direction == "mainlyZ") {
      if(oscOn && prevOsc=="x"){
        oscOff();
        oscOnZ();
      }else if(!oscOn){
...
function onCycle() {
  if ((typeof isSubSpindleCycle == "function") && isSubSpindleCycle(cycleType)) {
    ...
    if (!machineState.stockTransferIsActive) {
      if (cycleType != "secondary-spindle-return" && cycleType != "secondary-spindle-pull") {
        ...
        if(checkParting == "turningPart"){
          ...
          writeBlock("T" + toolFormat.format(partingToolCode), formatComment("call parting tool in preparation"));
        }
      }
      onCommand(COMMAND_STOP_SPINDLE);
      onCommand(COMMAND_COOLANT_OFF);
      ...
      if (getProperty("machineModel") == "PUMA_SMX") {
        ...
        writeBlock(feedMode, formatComment("feed mode"));
        writeBlock(gPlaneModal.format(18), formatComment("XZ plane"));
        writeBlock(cAxisEngageModal.format(getCode("DISABLE_C_AXIS", getSpindle(PART))), formatComment("Disable C axis"));
      }
      writeBlock(mFormat.format(getCode("DISABLE_C_AXIS", getSecondarySpindle())), formatComment("deactivate c axis sp2"));
    }
    ...
    case "secondary-spindle-grab":
      ...
      //Check that the next section is a bar pull and if so, move the parting tool across to the left
      var partingPark = Number(getNextSection().getParameter('pullingDistance'));
      partingPark = 1;
      if(partingPark){
        // writeComment(partingPark);
        writeBlock(gFormat.format(0), zOutput.format(partingPark), formatComment("parting blade park position"));
      }
      clampChuck(getSecondarySpindle(), UNCLAMP);
      onDwell(cycle.dwell);
      ...
      // clean out chips
      if (airCleanChuck) {
        writeBlock(mFormat.format(getCode("AIR_BLAST_ON", SPINDLE_MAIN)), formatComment("spindle 1 air blast on"));
        writeBlock(mFormat.format(getCode("AIR_BLAST_ON", SPINDLE_SUB)), formatComment("spindle 2 air blast on"));
        onDwell(3);
      }
      (getProperty("barLoader")==true)?writeBlock(mFormat.format(62), formatComment("signal bar feed")):"";
      writeBlock(mFormat.format(8), mFormat.format(478), formatComment("sub spindle coolant on"));
      // writeBlock(mInterferModal.format(getCode("INTERFERENCE_CHECK_OFF", getSpindle(PART))));
      ...
      gMotionModal.reset();
      moveSubSpindle(RAPID, cycle.feedPosition, 0, cycle.useMachineFrame, "rapid position sub spindle", true);
      ...
      if (transferUseTorque) {
        ...
        moveSubSpindle(FEED, cycle.chuckPosition, cycle.feedrate, cycle.useMachineFrame, "feed position sub spindle", true);
      }
      
      writeBlock(mFormat.format(getCode("AIR_BLAST_OFF", SPINDLE_MAIN)), formatComment("spindle 1 air blast off"));
...
function rigidTapping(x,y,z){
      mTappingModal.reset();
      // localZOutput.reset();
      var feedHeight = (currentSection.getParameter("operation:clearanceHeight"));
      // writeComment(cycle.clearance);
      var F = tool.getThreadPitch();
      //check if using a live tool in S3
      var liveTool = getSpindle(TOOL) == SPINDLE_LIVE;
      //check which spindle if tap is static
      var useSubSpindle = getSpindle(PART) == SPINDLE_SUB;
      //check orientation if live tap (X radial or Z axial)
      var toolOrientation = currentSection.getGlobalInitialToolAxis();
      // Check if tool is axial (pointing along Z-axis, parallel to spindle)
      var isAxial = (Math.abs(toolOrientation.z) > 0.9);
      //direction and axes to tap in
      var tapDir = 0;
      if(liveTool){
        if(isAxial){
          if(useSubSpindle){
      ...
      hOutput.reset();
      feedOutput.reset();
      // change cycle values depending on orientation
      var xStartPos = isAxial ? x : cycle.retract;
      var zStartPos = isAxial ? cycle.retract : z;
      var cStartPos;
      var holeBottom = isAxial ? z : 2*x;
      //holeBottom should get inverted when output as a zFormat in hOutput
      //if it is a radial hole in the subspindle, invert the value
      holeBottom = ((currentSection.spindle==SPINDLE_SECONDARY)&&!isAxial) ? -holeBottom : holeBottom;
      if (machineState.usePolarCoordinates) {
        var polarPosition = getPolarPosition(xStartPos, y, zStartPos);
        setCurrentPositionAndDirection(polarPosition);
        cOutput.reset();
        xStartPos = polarPosition.first.x;
        zStartPos = polarPosition.first.z;
        cStartPos = polarPosition.second.z;
      }
      writeComment(holeBottom);
      writeComment((currentSection.spindle==SPINDLE_SECONDARY));
      writeComment(hOutput.getScale());
      // writeCycleClearance(gPlaneModal.getCurrent(), cycle.clearance);
      writeBlock(
        xOutput.format(xStartPos), // x position
        (cStartPos)?cOutput.format(cStartPos):""
      );
      writeBlock(
        zOutput.format(zStartPos) // z top of hole
      );

      writeBlock(
        mTappingModal.format(329), //Tapping cycle
        // getCommonCycle(x, y, z, rapto, true),
        xOutput.format(xStartPos), // x position
        zOutput.format(zStartPos), // z top of hole
        (cStartPos)?cOutput.format(cStartPos):"",
        // // "H" + spatialFormat.format(z), // hole bottom
        hOutput.format(holeBottom),
        // rOutput.format(cycle.retract), // R point
        feedOutput.format(F), // pitch
        sOutput.format(spindleSpeed), // spindle speed
        "T" + tapDir
      );
      writeBlock(mFormat.format(304));
      
      writeBlock(
        (isAxial)?zOutput.format(cycle.clearance):xOutput.format(cycle.clearance) // z top of hole
      );
...
function onCyclePoint(x, y, z) {
  ...
  if(oscCutting){
    var direction = getLinearDirection(x, y, z);
    if (direction == "mainlyZ") {
      if(oscOn && prevOsc=="x"){
        oscOff();
        oscOnZ();
  ...
  case "thread-turning":
    if (getProperty("useSimpleThread") ||
      ...
      if (isLastCyclePoint()) {
        ...
        var chamferWidth = 0; // Pullout-width is 1*thread-lead in 1/10's;
  ...
  if (isFirstCyclePoint()) { // first cycle point
    ...
    case "left-tapping":
      ...
      // forceXYZ();
      // // change cycle values depending on orientation
      // var xStartPos = isAxial ? x : cycle.stock;
      // var zStartPos = isAxial ? cycle.stock : z;
      // var cStartPos;
      // var holeBottom = isAxial ? z : 2*x;
      // if (machineState.usePolarCoordinates) {
      //   var polarPosition = getPolarPosition(xStartPos, y, zStartPos);
    ...
    if (cycleExpanded) {
      ...
      if(cycleType=="tapping"){
        // writeBlock(getCommonCycle(x,y,z));
        rigidTapping(x,y,z);
      }else{
  ...
  if (Array.isArray(coolantCodes)) {
    if (singleLineCoolant) {
      ...
      for (var c in coolantCodes) {
        skipBlock = isOptionalCoolant;
        writeBlock(coolantCodes[c], formatComment("coolant"));
      }
    }
    return undefined;
  }
...
function moveSubSpindle(_method, _position, _feed, _useMachineFrame, _comment, _error) {
  ...
  case FEED:
    ...
    if (_useMachineFrame) {
      ...
      // writeComment("here");
      writeBlock(gFormat.format(getProperty("useG53ForXfer")==="g53"?53:54), gMotionModal.format(1), subOutput.format(_position), getFeed(_feed), conditional(_comment, formatComment(_comment)));
    }
    break;
  case TORQUE:
    gFeedModeModal.reset();
...
function onCommand(command) {
  ...
  case COMMAND_UNLOCK_MULTI_AXIS:
    ...
    if (unlockCode) {
      writeBlock(cAxisEngageModal.format(getCode("ENABLE_C_AXIS", getSpindle(PART))), formatComment("Enable c axis")); // C-axis must be enabled prior to unlocking it
      // writeBlock(unlockCode, formatComment("disable c axis"));
    }
    break;
  case COMMAND_START_CHIP_TRANSPORT:
    writeBlock(mFormat.format(200), formatComment("Start chip conveyor"));
    break;
  case COMMAND_STOP_CHIP_TRANSPORT:
    writeBlock(mFormat.format(201), formatComment("Stop chip conveyor"));
    break;
...
function ejectPart() {
  ...
  //Check if there is a bar pull section and if so, move the parting tool across to the left
  function getPullingDistance(defaultValue) {
    var numberOfSections = getNumberOfSections();
    for (var i = 0; i < numberOfSections; i++) {
        var section = getSection(i);
        if (section.getStrategy() == "turningSecondarySpindlePull") {
            if (section.hasParameter("operation:pullingDistance")) {
                return Number(section.getParameter("operation:pullingDistance"));
            }
        }
    }
  ...
  function partingTool(defaultValue) {
    var numberOfSections = getNumberOfSections();
    for (var i = 0; i < numberOfSections; i++) {
        var section = getSection(i);
        if (section.getStrategy() == "turningPart") {
            // if (section.getTool().number) {
            let toolNum = String(section.getTool().number);
            let toolComp = section.getTool().compensationOffset;
            var toolVal = toolNum.concat(toolComp);
                return toolVal; 
            // }
        }
    }
  ...
  var ejectPark = getPullingDistance(0); // Returns 50 if not found
  var partTool = partingTool(0); // Returns 50 if not found
  subOutput.reset();
  yOutput.reset();
  xOutput.reset();
  zOutput.reset();
  ...
  writeBlock(gFeedModeModal.format(getCode("FEED_MODE_UNIT_MIN", spindle)), formatComment("feedmode units per min"));
...
function engagePartCatcher(engage) {
  // writeComment("here");
  // writeComment(getProperty("partCatcherPosition"));
  if (getProperty("usePartCatcher")) {
    if (engage) { // engage part catcher
      writeBlock(mFormat.format(getCode("PART_CATCHER_ON", true)), formatComment(localize("PART CATCHER up to spindle")));
...
function onSectionEnd() {
  ...
  if (getProperty("cutoffConfirmation") != "false" && partCutoff && hasSubSpindle()) {
    ...
    case "m":
        writeBlock(mFormat.format(Number(cutoffCon.slice(1))), formatComment("CONFIRM CUTOFF"));
        break;
    case "g":
        writeBlock(gFormat.format(Number(cutoffCon.slice(1))), formatComment("CONFIRM CUTOFF"));
        break;
}
    onDwell(0.5);
  }
...
function onClose() {
  ...
  onCommand(COMMAND_STOP_SPINDLE);
  // var thisCycle = getSection(thisSectionID())
  if (currentSection.hasParameter("operation:cycleType")) {
    if (cycleType == "secondary-spindle-return") {
      writeBlock(mFormat.format(5), formatComment("turn off sp1 as well"));
    }
  }
  setCoolant(COOLANT_OFF);
  ...
  writeBlock(cAxisEngageModal.format(getCode("DISABLE_C_AXIS", getSpindle(PART))), formatComment("Deactivate c axis in both spindles"));
  ...
  if(!ejectRoutine&&getProperty("usePartCatcher")){
    if(hasPartOff() && !hasSubSpindle()){
      // onCommand(MAIN_)
      clampChuck(getSpindle(PART), UNCLAMP);
      writeBlock(mFormat.format(62), formatComment("signal to bar feeder. Must be in bar feed push mode"));
      clampChuck(getSpindle(PART), CLAMP);
    }
  }
...
function loadNewBar(){
  ...
  if (getProperty("barLoader")==true){
    ...
    //footer code to eject bar remnant and load new bar
    
    writeln("");
    if (getProperty("showSequenceNumbers") == "toolChange") {
      writeCommentSeqno(localize("New bar feed"));
    } else {
      writeComment(localize("New bar feed"));
    } 
    subOutput.reset();
    yOutput.reset();
    xOutput.reset();
    zOutput.reset();
    writeBlock("/3"+gFormat.format(53), gMotionModal.format(0), subOutput.format(getProperty("homePositionB")), formatComment("turret safe position b"));
    writeBlock("/3"+gFormat.format(53), gMotionModal.format(0), yOutput.format(getProperty("homePositionY")), formatComment("turret safe position y"));
    writeBlock("/3"+gFormat.format(53), gMotionModal.format(0), xOutput.format(getProperty("homePositionX")), formatComment("turret safe position x"));
    writeBlock("/3"+gFormat.format(53), gMotionModal.format(0), zOutput.format(-getProperty("homePositionZ")), formatComment("turret safe position z"));
    writeBlock("/3"+mFormat.format(5), mFormat.format(205), formatComment("stop spindles"));
    writeBlock("/3"+mFormat.format(73), formatComment("parts catcher up to chuck"));
    writeBlock("/3"+gFormat.format(4), xOutput.format(2), formatComment("dwell")); 
    writeBlock("/3"+mFormat.format(11), formatComment("main chuck unclamp"));
    writeBlock("/3"+gFormat.format(4), xOutput.format(2), formatComment("dwell"));
    writeBlock("/3"+mFormat.format(62), formatComment("signal to bar feeder"));
    writeBlock("/3"+mFormat.format(10), formatComment("main chuck clamp"));
    writeBlock("/3"+mFormat.format(74), formatComment("parts catcher down to conveyor"));
  }
